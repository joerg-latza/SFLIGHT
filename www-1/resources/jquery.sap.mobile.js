...
(function( $ ) {
	...
	if (mFakeFonts) { // this is only defined if we are in a valid fakeOS situation
		...
		$("head").append("<style>.sapUiBody{font-family:" + sFont + ",Helvetica,Arial !important;</style>");
	}
		...
		if (!_bInitMobileTriggered) { // only one initialization per HTML page
			...
			$(function() {
				...
				// check desktop browser
					...
					// check if browser is something else than webkit-based or IE10+
					if (!jQuery.browser.webkit && !(jQuery.browser.msie && jQuery.browser.fVersion >= 10)){
						sPopupWidth = "auto";
						bShowSupportMessage = true;
