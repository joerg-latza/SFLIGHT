...
sap.m.Carousel.prototype.onAfterRendering = function() {	
	...
	if(!this._oSwipeView) {
		...
		for (i=0; i<3; i++) {
			...
			this._aBusyIndicators[i] = new sap.m.BusyIndicator(this.getId() + "-indicator-" + i, {size: '6em'});
...
sap.m.Carousel.prototype.ontouchstart = function(oEvent) {
	...
	if(this._oSwipeView) {
		...
		if(oElement instanceof sap.m.Image || oElement instanceof sap.m.Button) {
			//Make an Exception for sap.m.Image and sap.m.Button. Ensure that horizontal swipe is 
			//performed anyway
...
sap.m.Carousel.prototype._addPage = function(oPage, iIndex) {
	...
	if(jQuery.sap.byId(oPage.getId()).length > 0) {
		jQuery.sap.assert(false, "sap.m.Carousel.prototype._addPage: Cannot add page '" + 
				oPage.getId() + "' because it already has been added'");
		return;
	}
...
sap.m.Carousel.prototype._doSwipeCompleted = function (oEvent, bInitialLoad) {
	...
	for (i=first; i<last; i++) {
		...
		if($ithMasterPage.hasClass("swipeview-active")) {
			...
			//hide neighboring pages after swipe is completed. Like this,
			//they are not mistakenly showing up, during orientation change
			this._oSwipeView.masterPages[i].style.visibility = 'hidden';
		}
	}
...
sap.m.Carousel.prototype._toggleBusyIcon = function(iMasterPage, bShowBusyIcon) {
	if(bShowBusyIcon) {
		...
		this._aMasterPageDivs[iMasterPage].css("visibility", "inherit");
	}
};
