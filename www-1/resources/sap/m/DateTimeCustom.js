...
(function($) {
	...
	if (sOS) {
		oDefaults.theme = (!$.os.ios) ? sOS + " light" : sOS;
	...
	$.extend(sap.m.DateTimeInput.prototype, {
			...
			var that = this,
				...
				oConfig = {
					...
					onShow : function(dialog) {
						...
						dialog.find(".dwbg .dwb").css("padding", "0 2px");
					},
					onClose : function() {
		...
		_getLongDatePattern : function(sPattern) {
			sPattern = (sPattern || this.getDisplayFormat()).replace(/y+/i, "YY");
			if (sOS == "wp") {	// for IE show day and month names to mimic native picker
				return sPattern.replace(/m+/i, "mm MM").replace(/d+/, "dd DD");
			} 
			return sPattern.replace(/m+/i, "MM").replace(/d+/, "dd");
		},
