...
sap.ui.core.Control.extend("sap.uiext.inbox.Inbox", { metadata : {
	...
	publicMethods : [
		// methods
		"clearDrillDownFilter", "clearRowSettings", "bindTaskTable", "getTaskTableBinding", "bindTaskExecutionURL", "bindTaskTypeDynamicFilter", "bindSearch", "setSubstitutionEnabled", "getSubstitutionEnabled", "bindTasks"
	],
 ...
 * @param {object} 
 *         oOFilter
 *         Predefined filter that is applied to the Task Collection before displaying the tasks in the Inbox. (sap.ui.ux3.TaskInitialFilters)
 * @param {object} 
 *         oOSorter
...
//      }
    ...
    if (sap.ui.getCore().byId(this.getId() + '--' + 'listViewTable') === undefined) {
        ...
        for ( var i = 0; i < sap.uiext.inbox.InboxConstants.mEntities.taskMetadata.properties.length; i++) {
            if (i == 0) {
            	var firstColMatLay;
            	if(sap.ui.getCore().byId(this.getId() + '--' + 'firstColumnMatLayout') === undefined){
            		firstColMatLay = new sap.ui.commons.layout.MatrixLayout({
					id : this.getId() + '--' + "firstColumnMatLayout",
					layoutFixed : false,
					width : "100%",
					columns : 4,
					widths : ['5%', '20%', '4%', '35%']
            		...
            		firstColMatLay = sap.ui.getCore().byId(this.getId() + '--' + 'firstColumnMatLayout');
            	}
            	var firstColMatRow = new sap.ui.commons.layout.MatrixLayoutRow({id : this.getId() + '--' + "firstColumnMatRow"}); 
            	var firstColMatCellOne = new sap.ui.commons.layout.MatrixLayoutCell({id : this.getId() + '--' + "firstColumnMatCellOne", hAlign : sap.ui.commons.layout.HAlign.Left  });
            	//add category image

            	var category_img = new sap.ui.commons.Image({
            		id : this.getId() + '--' + "tableCategoryImg",
            		decorative : false,
            		enable : false
            	});
            	category_img.bindProperty("src", that.constants.taskDefinitionNavigationParam, function(taskDefinitionData){
            		if(taskDefinitionData !== null && taskDefinitionData !== "" && taskDefinitionData !== undefined){
            			...
            			if (categoryValue !== null && categoryValue !== "" && categoryValue !== undefined) {
            				if(categoryValue === that._oBundle.getText("INBOX_TASK_CATEGORY")){
            					this.setTooltip(that._oBundle.getText("INBOX_TASK_CATEGORY"));
            					return that._imgResourcePath + sap.uiext.inbox.InboxConstants.taskCategoryImage;
            				}else if(categoryValue === that._oBundle.getText("INBOX_ALERT_CATEGORY")){
            					this.setTooltip(that._oBundle.getText("INBOX_ALERT_CATEGORY"));
            					return that._imgResourcePath + sap.uiext.inbox.InboxConstants.alertCategoryImage;
            				}else if(categoryValue == that._oBundle.getText("INBOX_NOTIFICATION_CATEGORY")){
            					this.setTooltip(that._oBundle.getText("INBOX_NOTIFICATION_CATEGORY"));
            					return that._imgResourcePath + sap.uiext.inbox.InboxConstants.notificationCategoryImage;
            				}
            			}
            		}
            		return that._imgResourcePath + sap.uiext.inbox.InboxConstants.taskCategoryImage;
            	}); 
            	firstColMatCellOne.addContent(category_img);
            	firstColMatCellOne.addStyleClass("sapUiExtInboxColumnTextLeftAlignment");
            	firstColMatRow.addCell(firstColMatCellOne);

            	//add task link
            	var firstColMatCellTwo = new sap.ui.commons.layout.MatrixLayoutCell({id : this.getId() + '--' + "firstColumnMatCellTwo", hAlign : sap.ui.commons.layout.HAlign.Left});
            	var taskLink = new sap.uiext.inbox.InboxLink();
                ...
                taskLink.attachPress(this, function(oEvent, inBox) { 
                								inBox._handleTaskTitleLinkPress(oEvent);
                					});
                firstColMatCellTwo.addStyleClass("sapUiExtInboxColumnTextLeftAlignment");
                firstColMatCellTwo.addContent(taskLink);
                firstColMatRow.addCell(firstColMatCellTwo);
                
				//add image for attachments
                var firstColMatCellThree = new sap.ui.commons.layout.MatrixLayoutCell({id : this.getId() + '--' + "firstColumnMatCellThree",hAlign : sap.ui.commons.layout.HAlign.Left});
				var tableAttachment_image = new sap.ui.commons.Image({
					id : this.getId() + '--' + "tableAttachmentImage",
					text : "",
					decorative : false,
					src : this._imgResourcePath + sap.uiext.inbox.InboxConstants.attachmentsImage
				});
				tableAttachment_image.bindProperty("visible", "HasAttachments", function(value) {
					if (value !== null && value !== "" && value !== undefined) {
						return value;
					}
					return false;
				});
				firstColMatCellThree.addContent(tableAttachment_image);
				firstColMatCellThree.addStyleClass("sapUiExtInboxColumnTextLeftAlignment");
				firstColMatRow.addCell(firstColMatCellThree);
				
				
				//add image for comments
				var firstColMatCellFour = new sap.ui.commons.layout.MatrixLayoutCell({id : this.getId() + '--' + "firstColumnMatCellFour",  hAlign : sap.ui.commons.layout.HAlign.Left});
				var tableCommentImage = new sap.ui.commons.Image({
					id : this.getId() + '--' + "tableCommentImage",
					text : "",
					decorative : false,
					src : this._imgResourcePath + sap.uiext.inbox.InboxConstants.commentsImage
				});
				tableCommentImage.bindProperty("visible", "HasComments", function(value) {
					if (value !== null && value !== "" && value !== undefined) {
						return value;
					}
					return false;
				});
				firstColMatCellFour.addContent(tableCommentImage);
				firstColMatCellFour.addStyleClass("sapUiExtInboxColumnTextLeftAlignment");
				firstColMatRow.addCell(firstColMatCellFour);
				
				firstColMatLay.addRow(firstColMatRow);
            ...
            // }).setWidth("20%").setLabel(new sap.ui.commons.Label({ //FIXME: Column  is not visible  
        				...
        				})).setTemplate(firstColMatLay).setSortProperty(sap.uiext.inbox.InboxConstants.mEntities.taskMetadata.properties[i])
			...
			taskInitiatorImageCell.addStyleClass("sapUiExtInboxRRFirstRowStyle");
	 	    ...
	 	    taskInitiatorImg.bindProperty("src", that.constants.taskDefinitionNavigationParam, function(taskDefinitionData){
	    		if(taskDefinitionData !== null && taskDefinitionData !== "" && taskDefinitionData !== undefined){
	    			...
	    			if (categoryValue !== null && categoryValue !== "" && categoryValue !== undefined) {
	    				if(categoryValue === that._oBundle.getText("INBOX_ALERT_CATEGORY"))
	    				...
	    				else if(categoryValue === that._oBundle.getText("INBOX_TASK_CATEGORY"))
...
//			         tooltip : this._oBundle.getText("INBOX_MSGBAR_CLOSE_TOOLTIP")
       ...
       return "";
		   ...
		   var taskInitiatorName = new sap.ui.commons.TextView(that.getId() + '--' + "taskInitiatorName",{design : sap.ui.commons.TextViewDesign.Small});
	 	   ...
	 	   dotLabel.bindProperty("visible","SupportsClaim",function(value){
	 		   	if(value!==undefined) {
	 		   		var oContext = this.getBindingContext();
	 		   		var bSupportsRelease = this.getModel().getProperty('SupportsRelease',oContext);
	 		   		var sStatus = this.getModel().getProperty('Status',oContext);
	 		   		value = (value && sStatus !== "COMPLETED")?true:false;
	 		   		var bShowSeparator = (value && bSupportsRelease)?true:false;
	    			return bShowSeparator;
	 		   	}
		    	return false;
	 	   });
	 	   ...
	 	   claimActionLink.bindProperty("visible","SupportsClaim",function(value){
		 	...
		 	releaseActionLink.bindProperty("visible","SupportsRelease",function(value){
		 		if(value!==undefined) {
	 		   		var oContext = this.getBindingContext();
	 		   		var bSupportsClaim = this.getModel().getProperty('SupportsClaim',oContext);
	 		   		var sStatus = this.getModel().getProperty('Status',oContext);
	 		   		value = (value && sStatus !== "COMPLETED")?true:false;
	    			return value;
	 		   	}
	 	 ...
	 	 //DOT Seperator b/w Release and Forward Action Links
		 	 ...
		 	 dotLabel2.bindProperty("visible","SupportsForward",function(value){if(value!==undefined) {
	 		   		var oContext = this.getBindingContext();
	 		   		var bSupportsRelease = this.getModel().getProperty('SupportsRelease',oContext);
	 		   		var bSupportsClaim = this.getModel().getProperty('SupportsClaim',oContext);
	 		   		var sStatus = this.getModel().getProperty('Status',oContext);
	 		   		value = (value && sStatus !== "COMPLETED")?true:false;
	 		   		var bShowSeparator = (value && (bSupportsRelease || bSupportsClaim))?true:false;
	    			return bShowSeparator;
	 		   	}
		    	return false;
		    });
...
sap.uiext.inbox.Inbox.prototype.bindTaskTable = function(sPath, oTaskFilters) {
	this.bindTasks(sPath, oTaskFilters);
};

/**
 * @param {string} sPath 
 * @param {sap.ui.ux3.TaskInitialFilters} oFilter
 * @param {sap.ui.model.Sorter} oSorter
 * 
 */
sap.uiext.inbox.Inbox.prototype.bindTasks = function(sPath, oFilter, oSorter) {
	var oTaskFilters = oFilter; 

	//Handle visibility of category
	if(!this.showTaskCategory){
		var tableCategoryImage = sap.ui.getCore().byId(this.getId() + '--' + 'tableCategoryImg');
    ...
    if (oTaskFilters !== undefined && oTaskFilters !== null) {
     ...
     //   if (this.defaultView === this.constants.tableView){
        	var expand = this._getExpandParameters();
        	if(expand.length > 0 ){
        		 table.bindRows({path: this.sCollectionPath
        				,parameters: {expand: expand},sorter:initialSort,filters:filtersTobeApplied});
        	}else{
	  ...
	  //  }
       ...
       // if (this.defaultView === this.constants.tableView){
        	...
        	if(expand.length > 0 ){
        		table.bindRows({path: this.sCollectionPath
        			,parameters: {expand: expand},sorter:initialSort,filters:[initialFilter]});
        	}else{
    ...
    for ( var j in oModel._original_data) {
        ...
        for ( var l in currentViewElement._originalKeys) {
            if (j == currentViewElement._originalKeys[l]) {
                ...
                if(!valuePresent){
                	if(val.Description !== undefined && val.Description !== null && val.Description !== ""){
                		var desc = val.Description;
                		descEntry = desc.__list[0];
                		descDataEntry = oModel.oData[descEntry];
                		var description;
                		if(descDataEntry){
                			description = descDataEntry.Description;
                		}
            	...
            	if(!valuePresent){
            		if(val.TaskDefinitionData !== undefined && val.TaskDefinitionData !== null && val.TaskDefinitionData !== ""){
            			...
            			if(category  && category.length > 0){
            				valuePresent = category.toLowerCase().indexOf(srchText) >= 0 ? true : false;
            			}
            		}
            	}
					...
					if(rrMatrixLayout){
							...
							if(buttonClickedID.indexOf(that.getId() + '--' + 'customAttributesSegBtn')!= -1){
								//if(this.data("previousSelection") !== buttonClickedID){
									var buttonClicked = oEvent.getSource();
									if(buttonClicked.getPressed()){
									//Show/Hide tooltip fix
										buttonClicked.setTooltip(that._oBundle.getText("INBOX_HIDE_CUSTOM_ATTRIBUTES"));
										...
										//Show/Hide tooltip fix
										buttonClicked.setTooltip(that._oBundle.getText("INBOX_SHOW_CUSTOM_ATTRIBUTES"));
									}
								//}
								//this.data("previousSelection",buttonClickedID);
...
sap.uiext.inbox.Inbox.prototype._executeForwardAction = function(oEvent, mParameters){
	...
	if(forwardTasksPopUp===undefined){
		...
		//	opener : sap.ui.getCore().byId(this.getId() + '--' + 'forwardActionButton'),
			buttons:[new sap.ui.commons.Button(dynamicId + "forwardTasksPopUpFrwBtn",{enabled:false, text: inbox._oBundle.getText("INBOX_ACTION_BUTTON_FORWARD"),press:function(pressEvent){
...
sap.uiext.inbox.Inbox.prototype._getExpandParameters = function(){
	...
	if(this.isCustomAttributesEnabled && (!this.initialLoad || isCurrentViewRowRepeater)){
		if(expand.length > 0){
			expand = expand + ",";
	    }
	...
	if(that.isForwardActionEnabled) {
		forwardActionLink.bindProperty("visible","SupportsForward",function(value){
			if(value!==undefined) {
 		   		...
 		   		var sStatus = this.getModel().getProperty('Status',oContext);
 		   		value = (value && sStatus !== "COMPLETED")?true:false;
 		   		/*var showSeparator = (value && (bSupportsRelease || bSupportsClaim))?true:false;
 		   		//TODO : make it generic using jQuery
 		   		var dotSeparator = this.getParent().getParent().getCells()[3].getContent()[0];
 		   		if(dotSeparator)
 		   			dotSeparator.setVisible(showSeparator);*/
    			/*if(sap.ui.getCore().byId(that.getId() + "--" + "dotSeparator2"))
    				sap.ui.getCore().byId(that.getId() + "--" + "dotSeparator2").setVisible(showSeparator);*/
    			return value;
 		   	}
	    	return false;
		});
...
sap.uiext.inbox.Inbox.prototype._clearCustomAttributeSort = function(){
    ...
    if(oBinding.sSortParams.indexOf(this.constants.customAttributeNavigationParam) !== -1){
    	oBinding.createSortParams(oBinding.oSorter);
    }
};
...
sap.uiext.inbox.Inbox.prototype._getTableIndicesByContext = function(aContexts,oTableElement){
	var aIndexes = [];
	var aBoundContextList = oTableElement.getBinding('rows').getContexts();
	...
	if(bTaskDefinitionFilter && !isCompletedTasksSelectedinDD){
		...
		if(aSelectedKeys && aSelectedKeys.length === 1){
			...
			this._deleteCustomActionsForTableView();
		            //create Columns
		    if(aCustomActionsDefinitionArray && aCustomActionsDefinitionArray.length>0)
		    	this._createCustomActions(aCustomActionsDefinitionArray, false);
		}else{
			this._deleteCustomActionsForTableView();
		}
	}else{
...
sap.uiext.inbox.Inbox.prototype._handleRequestCompleted = function(oEvent){
    ...
    if(!this._isTaskDefinitionFilterApplied() && this.bRefreshTaskTypes){
    ...
    oRefreshButton.setSrc(this._imgResourcePath + this.constants.refreshImage);
