...
sap.ui.richtexteditor.RichTextEditor.prototype.onAfterRenderingTinyMCE=function(){if(!this._bEditorCreated){if(!sap.ui.richtexteditor.RichTextEditorRenderer.bTinyMCELicenseMissing){this.initTinyMCEAfterFirstRendering()}}else{var i=tinymce.getInstanceById(this.textAreaId);var v;if(i&&(v=this.getValue())){i.setContent(v)}if(window.tinymce){tinymce.execCommand('mceAddControl',false,this.textAreaId)}}};
sap.ui.richtexteditor.RichTextEditor.prototype.initTinyMCEAfterFirstRendering=function(){if(!sap.ui.richtexteditor.RichTextEditor.TinyMCEInitialized){sap.ui.richtexteditor.RichTextEditor.initTinyMCEStatic()}if(!window.tinymce){jQuery.sap.delayedCall(10,this,"initTinyMCEAfterFirstRendering");return}this._bEditorCreated=true;var g=this.getShowGroupFontStyle()?sap.ui.richtexteditor.RichTextEditor.TinyMCE.GroupFontStyle:"";var a=this.getShowGroupTextAlign()?sap.ui.richtexteditor.RichTextEditor.TinyMCE.GroupTextAlign:"";var b=this.getShowGroupFont()?sap.ui.richtexteditor.RichTextEditor.TinyMCE.GroupFont:"";var c=this.getShowGroupClipboard()?sap.ui.richtexteditor.RichTextEditor.TinyMCE.GroupClipboard:"";var d=this.getShowGroupStructure()?sap.ui.richtexteditor.RichTextEditor.TinyMCE.GroupStructure:"";var e=this.getShowGroupInsert()?sap.ui.richtexteditor.RichTextEditor.TinyMCE.GroupInsert:"";var f=this.getShowGroupLink()?sap.ui.richtexteditor.RichTextEditor.TinyMCE.GroupLink:"";var h=this.getShowGroupUndo()?sap.ui.richtexteditor.RichTextEditor.TinyMCE.GroupUndo:"";var i=g+",|,"+a+",|,"+b;i=i.replace(/,(,)+/g,",").replace(/\|(,\|)+/g,"|").replace(/^,?\|,/,"").replace(/,\|,?$/,"");var j=c+",|,"+d+",|,"+h+",|,"+e+",|,"+f;j=j.replace(/,(,)+/g,",").replace(/\|(,\|)+/g,"|").replace(/^,?\|,/,"").replace(/,\|,?$/,"");if(!i||i==""){i=j;j=null}var t=this;var D=(sap.ui.getCore().getConfiguration().getRTL()?"rtl":"ltr");var l=new sap.ui.core.Locale(sap.ui.getCore().getConfiguration().getLanguage()),L=l.getLanguage();if(L=="zh"){L+="-"+l.getRegion().toLowerCase()}if(L=="sh"){L="sr"}var C={mode:"exact",directionality:D,elements:this.getId()+'-textarea',theme:"advanced",height:this.getHeight(),language:L,plugins:"emotions,directionality,inlinepopups",theme_advanced_buttons1:i,theme_advanced_buttons2:j,theme_advanced_buttons3:null,theme_advanced_buttons4:null,theme_advanced_toolbar_location:"top",theme_advanced_toolbar_align:"left",theme_advanced_statusbar_location:"none",onchange_callback:function(o){t.onTinyMCEChange(o)}};C.readonly=(this.getEditable()?0:1);C.nowrap=!this.getWrapping();tinymce.init(C);var k=tinymce.getInstanceById(this.textAreaId);var v;if(k&&(v=this.getValue())){k.setContent(v)}this.initWhenTinyMCEReady();this._tinyMCEPreserveHandler=function(s,E,o){if((t.getDomRef()&&window.tinymce&&jQuery(o.domNode).find(jQuery.sap.byId(t.textAreaId)).length>0)||(jQuery.sap.byId(t.textAreaId).length==0)){var k=tinymce.getInstanceById(t.textAreaId);if(k){t.setProperty("value",k.getContent(),true)}tinymce.execCommand('mceRemoveControl',false,t.textAreaId)}};sap.ui.getCore().getEventBus().subscribe("sap.ui","__preserveContent",this._tinyMCEPreserveHandler)};
sap.ui.richtexteditor.RichTextEditor.prototype.initWhenTinyMCEReady=function(){if(!this.tinyMCEReady()){jQuery.sap.delayedCall(100,this,"initWhenTinyMCEReady");return}var e=tinymce.getInstanceById(this.textAreaId);if(this.getTooltip()&&this.getTooltip().length>0){var t=jQuery.sap.escapeHTML(this.getTooltip_Text());e.getBody().title=t;jQuery.sap.byId(this.textAreaId+"_ifr").attr("title",t)}var a=this.textAreaId+"_tbl";var E=jQuery.sap.byId(a);this.$focusables=E.find(":sapFocusable");E.bind('keydown',jQuery.proxy(this,"_tinyMCEKeyboardHandler"));jQuery.sap.byId(this.getId()+"-textarea_fontselect").attr("title","Font");jQuery.sap.byId(this.getId()+"-textarea_fontsizeselect").attr("title","Font Size");this.fireReady()};
...
sap.ui.richtexteditor.RichTextEditor.prototype.setValueTinyMCE=function(v){if(v||v==""){if(this._bEditorCreated){jQuery.sap.byId(this.textAreaId).text(v);var i=tinymce.getInstanceById(this.textAreaId);if(i){i.setContent(v)}}else{this.setProperty("value",v,true);if(this.getDomRef()){jQuery.sap.byId(this.textAreaId).val(v)}}}};
...
sap.ui.richtexteditor.RichTextEditor.prototype.reinitializeTinyMCE=function(){if(this._bEditorCreated){this._bEditorCreated=false;this.rerender();var i=tinymce.getInstanceById(this.textAreaId);var v;if(i&&(v=this.getValue())){i.setContent(v)}}};
sap.ui.richtexteditor.RichTextEditor.prototype.exitTinyMCE=function(){this.bExiting=true;if(tinymce){tinymce.execCommand('mceRemoveControl',false,this.textAreaId)}sap.ui.getCore().getEventBus().unsubscribe("sap.ui","__preserveContent",this._tinyMCEPreserveHandler)};
