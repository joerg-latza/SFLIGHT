 ...
 * <li>{@link #getFixedRowCount fixedRowCount} : int (default: 0)</li></ul>
 * </li>
 * <li>Aggregations
 * <ul>
 ...
 * This defines how the table handles the visible rows in the table. The default behavior is, that a fixed row count is defined. If you change it to auto the visibleRowCount property is changed by the table automatically. In manual mode the user can change the visibleRowCount interactively
 *
...
sap.ui.table.Table.prototype._updateTableContent = function() {
	...
	if (this._modifyRow) {
		...
		jQuery.each(this.getRows(), function(iIndex, oRow) {
			...
			that._modifyRow(iIndex + that.getFirstVisibleRow(), jQuery.sap.byId(oRow.getId() + "-fixed"));
		});
	}
...
sap.ui.table.Table.prototype._initItemNavigation = function() {
	...
	if (this.getFixedColumnCount() == 0) {
		...
		var $bottomLeft = this.$().find('.sapUiTableCtrlFixed.sapUiTableCtrlRowScroll');
		var $bottomRight = this.$().find('.sapUiTableCtrlScroll.sapUiTableCtrlRowScroll');
...
sap.ui.table.Table.prototype._attachEvents = function() {
	...
	$this.find(".sapUiTableRowHdr").hover(function() {
		...
		$this.find(".sapUiTableCtrl > tbody > tr").filter(":eq(" + iIndex + ")").addClass("sapUiTableRowHvr");
	}, function() {
...
sap.ui.table.Table.prototype._createRows = function(iStartIndex) {
	...
	if (oBinding) {
		aContexts = oBinding.getContexts(iStartIndex, iVisibleRowCount, Math.max(this.getVisibleRowCount(), this.getThreshold()));
	}
...
sap.ui.table.Table.prototype._updateBindingContexts = function() {
	...
	if (oBinding) {
		if (iFixedRows > 0) {
			aFixedContexts = oBinding.getContexts(0, iFixedRows, Math.max(iFixedRows, this.getThreshold()));
			aContexts = oBinding.getContexts(this.getFirstVisibleRow() + iFixedRows, aRows.length - iFixedRows, Math.max((this.getVisibleRowCount() - iFixedRows), this.getThreshold()));
	...
	for (var iIndex = aRows.length - 1; iIndex >= 0; iIndex--) {
		...
		if (oRow) {
			this._updateRowBindingContext(oRow, oContext, oBindinginfo.model);
		}
	}
...
sap.ui.table.Table.prototype._updateRowBindingContext = function(oRow, oContext, sModelName) {
	...
	if (oContext && oContext instanceof sap.ui.model.Context) {
		for (var i = 0, l = this._aVisibleColumns.length; i < l; i++) {
			var col = this._aVisibleColumns[i];
...
sap.ui.table.Table.prototype._determineVisibleCols = function() {
	...
	if ($this.hasClass("sapUiTableHScr")) {
		...
		this._aVisibleColumns = [];
		for (var i = 0, l = this._getVisibleColumnCount(); i < l; i++) {
			this._aVisibleColumns.push(i);
		};
	}
...
sap.ui.table.Table.prototype._updateRowHeader = function() {
	...
	for (var i = 0, l = $scrollRows.length; i < l; i++) {
		var iHeight = Math.max(jQuery($fixedRows[i]).height(), jQuery($scrollRows[i]).height());
		if (this._bjQueryLess18) {
	...
	if (iFixedRows > 0) {
		var iOffsetTop = $rowHeaders.filter(':eq(' + iFixedRows + ')')[0].offsetTop;
...
sap.ui.table.Table.prototype.onmousedown = function(oEvent) {
	...
	var $splitter = jQuery.sap.byId(this.getId() + "-sb");
...
sap.ui.table.Table.prototype._onRowSelect = function(iRowIndex, bShift, bCtrl) {
	...
	if (oSelMode !== sap.ui.table.SelectionMode.None) {
		if (oSelMode === sap.ui.table.SelectionMode.Single) {
			...
			if (bShift) {
				//If no row is selected getSelectedIndex returns -1. We cannot use add selection in this case
				//therefore we assume, that the first visible item is start of the selection
				this.addSelectionInterval(Math.max(this.getFirstVisibleRow(), this.getSelectedIndex()), iRowIndex);
			} else {
				if (!this.isIndexSelected(iRowIndex)) {
					if (bCtrl) {
	...
	$this.find(".sapUiTableRowHdr").each(function(iIndex, oElement) {
		...
		if (that.isIndexSelected(iFirstRow + iIndex)) {
			if (!jQuery(this).hasClass("sapUiTableRowSel")) {
				...
				$jqTR1.children("td").attr("aria-selected", "true");
				$jqTR2.children("td").attr("aria-selected", "true");
			}
			if (bMultiSelection) {
				$jqRow.attr("title", oResBundle.getText("TBL_ROW_DESELECT_MULTI")).
				  attr("aria-label", oResBundle.getText("TBL_ROW_DESELECT_MULTI_KEY"));
				$jqTR1.find(".sapUiTableAriaRowSel").text(oResBundle.getText("TBL_ROW_DESELECT_MULTI_KEY"));
				$jqTR2.find(".sapUiTableAriaRowSel").text(oResBundle.getText("TBL_ROW_DESELECT_MULTI_KEY"));
			} else {
				$jqRow.attr("title", oResBundle.getText("TBL_ROW_DESELECT")).
					attr("aria-label", oResBundle.getText("TBL_ROW_DESELECT_KEY"));
				$jqTR1.find(".sapUiTableAriaRowSel").text(oResBundle.getText("TBL_ROW_DESELECT_KEY"));
				$jqTR2.find(".sapUiTableAriaRowSel").text(oResBundle.getText("TBL_ROW_DESELECT_KEY"));
			}
		} else {
			if (jQuery(this).hasClass("sapUiTableRowSel")) {
				...
				if (that.getSelectionMode()  === sap.ui.table.SelectionMode.Multi) {
					...
					$jqTR1.children("td").attr("aria-selected", "false");
					$jqTR2.children("td").attr("aria-selected", "false");
				} else {
					$jqRow.removeAttr("aria-selected");
					$jqTR1.children("td").removeAttr("aria-selected");
					$jqTR2.children("td").removeAttr("aria-selected");
				}
				$jqRow.attr("title", oResBundle.getText("TBL_ROW_SELECT")).
				  attr("aria-label", oResBundle.getText("TBL_ROW_SELECT_KEY"));
				$jqTR1.find(".sapUiTableAriaRowSel").text(oResBundle.getText("TBL_ROW_SELECT_KEY"));
				$jqTR2.find(".sapUiTableAriaRowSel").text(oResBundle.getText("TBL_ROW_SELECT_KEY"));
			}
		}
	});
...
sap.ui.table.Table.prototype.onsapselect = function(oEvent) {
	if (oEvent.srcControl !== this && jQuery.inArray(oEvent.srcControl,this.getRows()) === -1) {
		return;
	}
	this._bShowMenu = true;
	this._onSelect(oEvent);
...
sap.ui.table.Table.prototype.onkeydown = function(oEvent) {
	...
	if (!this._bActionMode &&
		...
		if ($this.find(".sapUiTableCtrl td:focus").length > 0) {
			this._enterActionMode($this.find(".sapUiTableCtrl td:focus :sapFocusable").get(0));
			oEvent.preventDefault();
			oEvent.stopPropagation();
		}
		...
		if (this.getFixedColumnCount() > 0) {
			...
			var $table = $row.parents("table[role=grid]");
...
sap.ui.table.Table.prototype._onGhostMouseRelease = function(oEvent) {
	...
	var splitterBarGhost = jQuery.sap.domById(this.getId() + "-ghost");
...
sap.ui.table.Table.prototype._calculateRowsToDisplay = function(iHeight) {
	...
	return Math.max(5, Math.floor(iAvailableSpace/iRowHeight));
};
