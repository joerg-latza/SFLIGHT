...
(function() {
	...
	sap.ui.core.plugin.LessSupport.prototype.startPlugin = function(oCore, bOnInit) {
		...
		if (window.top.JsUnit || (window.sap.ui.test && window.sap.ui.test.qunit)) {
		...
		function checkThemeApplied(){
			...
			for(var lib in mLibs){
				check = sap.ui.core.ThemeCheck.checkStyle("less:"+lib, true);
				if(check){
					jQuery.sap.byId("sap-ui-theme-"+lib).attr("sap-ui-ready", "true");
				}
				ok = ok && check;
			}
