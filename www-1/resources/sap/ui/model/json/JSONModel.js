...
sap.ui.model.json.JSONModel.prototype.loadData=function(u,p,a,t,m,c){var b=this;if(a!==false){a=true}if(!t){t="GET"}if(c===undefined){c=this.bCache}this.fireRequestSent({url:u,type:t,async:a,info:"cache="+c+";bMerge="+m});jQuery.ajax({url:u,async:a,dataType:'json',cache:c,data:p,type:t,success:function(d){if(!d){jQuery.sap.log.fatal("The following problem occurred: No data was retrieved by service: "+u)}b.setData(d,m);b.fireRequestCompleted({url:u,type:t,async:a,info:"cache=false;bMerge="+m})},error:function(X,d,e){jQuery.sap.log.fatal("The following problem occurred: "+d,X.responseText+","+X.status+","+X.statusText);b.fireRequestCompleted({url:u,type:t,async:a,info:"cache=false;bMerge="+m});b.fireRequestFailed({message:d,statusCode:X.status,statusText:X.statusText,responseText:X.responseText})}})};
