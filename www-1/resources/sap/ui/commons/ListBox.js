...
sap.ui.commons.ListBox.prototype.init = function () {
	...
	sap.ui.core.LocalBusyIndicatorSupport
			.apply(sap.ui.commons.ListBox.prototype);
};
...
sap.ui.commons.ListBox.prototype.onmousedown = function(oEvent) {
	if (jQuery.browser.webkit && oEvent.target && oEvent.target.id === this.getId()) { // ListBox scrollbar has been clicked; webkit completely removes the focus, which breaks autoclose popups
 ...
 * @param bNoItemsChanged not in official API, only needen in DropdownBox TypeAhead
 */
 ...
 * inform ComboBox if an item has cahnged
*/
sap.ui.commons.ListBox.prototype.invalidate = function(oOrigin) {

	sap.ui.core.Control.prototype.invalidate.apply(this, arguments);

	if(oOrigin || oOrigin instanceof sap.ui.core.Item){
		// item has changed - but fire event only if item changed itself and not only the aggregation
		if (!this.bNoItemInvalidateEvent) {
			this.fireEvent("itemInvalidated", {item: oOrigin}); //private event used in ComboBox
		}
	}
