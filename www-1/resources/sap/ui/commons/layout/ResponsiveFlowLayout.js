...
(function() {
	...
	var updateRows = function(oThis) {
		...
		for ( var i = 0; i < aControls.length; i++) {
			...
			var bMargin = sap.ui.commons.layout.ResponsiveFlowLayoutData.MARGIN;
		...
		if (this.getDomRef) {
			...
			if (rows) {
				for ( var i = 0; i < rows.length; i++) {
					...
					if (bInitial || rows[i].height != $row.height() || checkWrappingDiff(oCurrentWrapping, oTargetWrapping)) {
