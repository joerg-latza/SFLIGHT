...
(function() {
	...
	var fnSanitizeAttribs = function(tagName, attribs) {
		for ( var i = 0; i < attribs.length; i += 2) {
			...
			if (this._ftv.ATTRIBS[sAttribKey]) {
				...
				var sWarning = '<' + tagName + '> with attribute ['
						+ attribs[i] + '="' + attribs[i + 1]
						+ '"] is not allowed and cut';
	...
	var fnPolicy = function(tagName, attribs) {
		if (this._ftv.ELEMENTS[tagName]) {
			...
			var sWarning = '<' + tagName
					+ '> is not allowed and cut (and its content)';
			jQuery.sap.log.warning(sWarning, this);
		}
	};
	...
	sap.ui.commons.FormattedTextView.prototype.setContent = function(sHtmlText,
			aControls) {
