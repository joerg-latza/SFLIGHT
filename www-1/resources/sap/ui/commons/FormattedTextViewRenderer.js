...
/*
 * Renders the HTML for the FormattedTextView, using the provided {@link sap.ui.core.RenderManager}.
 *
...
sap.ui.commons.FormattedTextViewRenderer.render = function(oRm, oControl) {
	...
	var rPlaceHolderPattern = /<embed\s+data-index="([0-9]+)"\s*\/?>/gim,
			sHtml = oControl.getHtmlText(),
			aInnerControls = oControl.getControls().slice(), // clone, will be modified below
			iToBeRendered = aInnerControls.length,
			lastIndex = 0,
			aMatch;
	...
	while (aMatch = rPlaceHolderPattern.exec(sHtml)) {
		// write any static HTML between previous and current placeholder (if any) 
	...
	oRm.write(sHtml.slice(lastIndex, sHtml.length)); // render the remainder of the HTML
...
sap.ui.commons.FormattedTextViewRenderer._renderReplacement = function(rm, controlsIndex, aControls) {
	if (aControls[controlsIndex]) {
		...
		aControls[controlsIndex] = null; // UI5 controls must not be rendered twice! 
		return true;
	} else
		return false;
};