...
sap.ui.commons.TextField.prototype.onkeypress = function(oEvent) {
	...
	var iKC = oEvent.keyCode;
	if(!( iKC >= 33 && iKC <= 44) /*arrow keys...*/ && !( iKC >= 112 && iKC <= 123)/*F-keys*/ &&
	   iKC !== jQuery.sap.KeyCodes.ESCAPE && iKC !== jQuery.sap.KeyCodes.TAB){
...
/*
 * sets the old value after escape in case of firefox
 * @private
 */
sap.ui.commons.TextField.prototype._doOnEscape = function() {

	if (this._bEsc) {
		// restore old value in case of escape key (not possible in onsapescape)
		var oInput = this.getInputDomRef();
		if(oInput && oInput.value !== this._sValue) {
