...
sap.ui.commons.Menu.prototype.open = function(bWithKeyboard, oOpenerRef, my, at, of, offset, collision){
	...
	if(jQuery.support.orientation && this.getRootMenu() === this){
...
jQuery.ui.position._sapUiCommonsMenuFlip = {
	left: function(position, data){
		...
		over = data.collisionPosition.left < 0 ? Math.abs(data.collisionPosition.left) : over;
		
		if(over > 0){
			correction = myOffset + atOffset + offset;
			var over2 = data.collisionPosition.left + correction;
			
			if(correction > 0){ //Move to right
				over2 += data.collisionWidth - win.width() - win.scrollLeft();
				skipCorrection = over2 > 0 && Math.abs(over2) > over;
			}else if(correction < 0){ //Move to left
				skipCorrection = over2 < 0 && Math.abs(over2) > over;
			}
			
			correction = skipCorrection ? 0 : correction;
		}
	...
	top: function(position, data){
		...
		if(data.collisionPosition.top < 0 || over > 0) {
			correction = myOffset + atOffset + offset;
			var over2 = data.collisionPosition.top + correction;
			
			if(over2 < 0 && over > 0 && Math.abs(over2) > over){
				correction = 0;
			}
		}
		position.top += correction;
	}
};