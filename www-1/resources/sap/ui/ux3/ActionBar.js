				...
				if(oActionBar.mActionMap[oActionBar.mActionKeys.Follow]) {
					...
					if (!oActionBar._oMenu) {
						...
						oActionBar._oMenu.attachItemSelect(jQuery.proxy(function(oControlEvent) {
							this._fnFollowMenuSelected(oEvent, oActionBar);
						}, this));
...
sap.ui.ux3.ActionBar.prototype._addBusinessAction = function(oBusinessAction, iIndex) {
	...
	//Make sure that business actions are re-rendered if their ThingAction properties change
	oBusinessAction.setEnabled = function(flag) {
		sap.ui.ux3.ThingAction.prototype.setProperty.call(oBusinessAction, "enabled", flag, true);
	}; 
	
	oBusinessAction.setText = function(text) {
		sap.ui.ux3.ThingAction.prototype.setProperty.call(oBusinessAction, "text", text, true);
	}; 
	...
	//listen to changes of business action properties (e.g. enabled, text)
	oMenuItem.actionPropertyChanged = function(oEvent) {
		this.setProperty(oEvent.getParameter("name"), oEvent.getParameter("newValue"));
		
	};
	oBusinessAction.attachEvent("_change", oMenuItem.actionPropertyChanged, oMenuItem);
...
sap.ui.ux3.ActionBar.prototype._onresize = function(oEvent) {
	...
	if(!this.getAlwaysShowMoreMenu() && this._oMoreMenuButton ) {
		...
		bShowMoreMenuButton ? oMoreMenuButtonDomRef.show() : oMoreMenuButtonDomRef.hide();
...
sap.ui.ux3.ActionBar.prototype._createButtonForAction = function(oBusinessAction, oMenuItem, iIndex) {
	if(!this.getAlwaysShowMoreMenu() && !oBusinessAction.showInMoreMenu) {
		...
		//listen to changes of business action properties (e.g. enabled, text)
		oButton.actionPropertyChanged = function(oEvent) {
			this.setProperty(oEvent.getParameter("name"), oEvent.getParameter("newValue"), true);
			that._rerenderBusinessAction(this);
			
		};
		oBusinessAction.attachEvent("_change", oButton.actionPropertyChanged, oButton);
