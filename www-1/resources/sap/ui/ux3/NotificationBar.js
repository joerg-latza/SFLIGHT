...
/**
 * This event is thrown when the bar was resized. 
 *
...
(function() {
	...
	var fnChildEventListener = function(oEvent) {
		...
		case "openCallout":
			...
			var oNotifierView = new sap.ui.ux3.NotificationBar.NotifierView({
	...
	sap.ui.ux3.NotificationBar.prototype.init = function() {
		 ...
		 * the NotificationBat is not used top level (e.g. in Shell) but in some
		 ...
		 * Cons (ResizeHandler does some kind of polling which is not nasty...)
		 */
	...
	var fnCreateMessageView = function(oMessage, oNotifier, oNotiBar) {
		var oMessageView = new sap.ui.ux3.NotificationBar.MessageView({
	...
	sap.ui.ux3.NotificationBar.prototype.setMessageNotifier = function(oMessageNotifier) {
		...
		if (oMessageNotifier) {
			oMessageNotifier._oMessageArea = new sap.ui.ux3.NotificationBar.MessageView();
	 ...
	 * @private
	 */
	sap.ui.ux3.NotificationBar.prototype.onBeforeRendering = function() {
		if (jQuery.browser.msie) {
			/*
			 * This crap is needed since the IE has problems to show the bar
			 * again if the was was minimized and the user clicked on 'barUp'.
			 * Only if the mouse was moved the IE shows a previously hidden
			 * element again.
			 */
			window.parent.document.mousePageY = 0;
		}
	};
	...
	sap.ui.ux3.NotificationBar.prototype.getHeightOfStatus = function(sStatus) {
		var sParamRoot = "sap.ui.ux3.NotificationBar:";
		var sParam = "";

		if (sStatus == sap.ui.ux3.NotificationBarStatus.Min) {
			sParam = sParamRoot + "sapUiNotificationBarHeightMinimized";
		} else if (sStatus == sap.ui.ux3.NotificationBarStatus.Default) {
			sParam = sParamRoot + "sapUiNotificationBarHeight";
		} else if (sStatus == sap.ui.ux3.NotificationBarStatus.Max) {
			sParam = sParamRoot + "sapUiNotificationBarHeightMaximized";
