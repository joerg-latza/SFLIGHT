 ...
 * <li>{@link #getFullHeightContent fullHeightContent} : boolean (default: false)</li></ul>
 * </li>
 * <li>Aggregations
 * <ul>
...
sap.ui.ux3.Shell.prototype.onAfterRendering = function() {
	...
	this._iFacetBarHeight = $FacetBar.outerHeight(true);
		...
		if (id != this._sSelectedWorksetId) {   // it's actually a change
			...
			if (bProcessSelection) {
				...
				if (aSubItems.length > 0) {
					...
					this._adaptContentHeight(null, true);
				} else {
...
(function() {
	...
	sap.ui.ux3.Shell.prototype._closePane = function(fCompleteCallback) {
		...
		var id = this.getId();
		var dist = this.getShowPane() ? (sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) : sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT;
		var props = {};
		props[this._bRtl ? "left" : "right"] = dist + "px";
		var props2 = {};
		props2[this._bRtl ? "marginLeft" : "marginRight"] = (dist + 14 - sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px"; //TODO: Theme Parameter
		...
		jQuery.sap.byId(id + "-pb_" + this._sOpenPaneId).removeClass("sapUiUx3ShellPaneEntrySelected");
	...
	sap.ui.ux3.Shell.prototype._openPane = function() {
		...
		if (this._bRtl) {
			...
			jQuery.sap.byId(id+"-wBar").stop().animate({marginLeft: (PANE_WIDTH + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + 22) + "px"});
			jQuery.sap.byId(id+"-canvas").stop().animate({left: (PANE_WIDTH + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px"});
			jQuery.sap.byId(id + "-notify").stop().animate({left: (PANE_WIDTH + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px"});
			this._getSyncRefs().stop(false, true).animate({left: (PANE_WIDTH + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px"});
			jQuery.sap.byId(id+"-canvasBackground").stop().animate({left: (PANE_WIDTH + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px"}, function() {
				jQuery.sap.byId(id + "-content").css("overflow-x", "");
			...
			jQuery.sap.byId(id+"-paneBar").addClass("sapUiUx3ShellPaneBarOpen");
			jQuery.sap.byId(id+"-wBar").stop().animate({marginRight: (PANE_WIDTH + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + 22) + "px"});
			jQuery.sap.byId(id+"-canvas").stop().animate({right: (PANE_WIDTH + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px"});
			jQuery.sap.byId(id + "-notify").stop().animate({right: (PANE_WIDTH + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px"});
			this._getSyncRefs().stop(false, true).animate({right: (PANE_WIDTH + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px"});
			jQuery.sap.byId(id+"-canvasBackground").stop().animate({right: (PANE_WIDTH + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px"}, function() {
				jQuery.sap.byId(id + "-content").css("overflow-x", "");
				jQuery.sap.byId(id+"-canvasBackground").removeClass("sapUiUx3ShellCanvasBackgroundClosed").addClass("sapUiUx3ShellCanvasBackgroundOpen");
			});
		}
	...
	sap.ui.ux3.Shell.prototype.setPaneWidth = function (iWidth) {
		if (typeof iWidth == "number" && (iWidth > 0)) {
			...
			if (this.getDomRef()) {
				...
				jQuery.sap.byId(id + "-paneBar").css("width", (iWidth + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE) + "px");

				if (!!this._sOpenPaneId) { // pane area is open
					if (this._bRtl) {
						jQuery.sap.byId(id+"-wBar").css("marginLeft", (iWidth + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + 22) + "px");
						jQuery.sap.byId(id+"-canvas").css("left", (iWidth + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px");
						jQuery.sap.byId(id+"-notify").css("left", (iWidth + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px");
						this._getSyncRefs().css("left", (iWidth + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px");
						jQuery.sap.byId(id+"-canvasBackground").css("left", (iWidth + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px");
					} else {
						jQuery.sap.byId(id+"-wBar").css("marginRight", (iWidth + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + 22) + "px");
						jQuery.sap.byId(id+"-canvas").css("right", (iWidth + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px");
						jQuery.sap.byId(id+"-notify").css("right", (iWidth + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px");
						this._getSyncRefs().css("right", (iWidth + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px");
						jQuery.sap.byId(id+"-canvasBackground").css("right", (iWidth + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px");
					}
				}
			}
	...
	sap.ui.ux3.Shell.prototype.setOffsetRight = function(px, complete, outerId) {
		...
		if(!this.$().hasClass("sapUiUx3ShellNoPane")){
			pxVal = (px + (this._sOpenPaneId ? this.getPaneWidth() : 0) + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE) + "px";
		}
	...
	sap.ui.ux3.Shell.prototype._refreshCanvasOffsetRight = function(bPaneShown) {
		...
		if(bPaneShown){
			pxVal = pxVal + (this._sOpenPaneId ? this.getPaneWidth() : 0) + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE;
		}
		pxVal = pxVal + "px";
	...
	sap.ui.ux3.Shell.prototype._setNotifyVisibility = function(sVisibleStatus) {
		...
		if(this._topSyncRefId){
			this._getSyncRefs().stop(true, true).animate({"bottom" : iBottom+"px"}, "fast");
 ...
 *
 * Only to be used internally (by the ThingInspector/OverlayContainer)! Marking as "private", but actually is "package" visibility.
 * @private
 */
sap.ui.ux3.Shell.prototype.syncWithCanvasSize = function(sId, bInit, fFocusFirst, fFocusLast) {
	var idx = jQuery.inArray(sId, this._aSyncRefStack);
	
	//Update sync ref data structure
	if(bInit){
		var oRef = {id: sId, focusFirst: fFocusFirst, focusLast: fFocusLast};
		if(idx < 0){
	...
	if(this._aSyncRefStack.length > 0){
		...
		var $DomRef = jQuery.sap.byId(oRef.id);
		var oCanvas = jQuery.sap.domById(this.getId() + "-canvas");
		$DomRef.css(this._bRtl ? "right" : "left", (this.getShowTools() ? sap.ui.ux3.Shell._PANE_OUTER_DISTANCE : 0) + "px");
		var iRight = sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT;
		if(this.getShowPane()){
			iRight += sap.ui.ux3.Shell._PANE_OUTER_DISTANCE;
			if(this._sOpenPaneId){
				iRight += this.getPaneWidth();
			}
		}
		$DomRef.css(this._bRtl ? "left" : "right", iRight + "px");
		$DomRef.css("top", jQuery.sap.domById(this.getId() + "-hdrLine").offsetHeight + "px");
		$DomRef.css("bottom", "0");
		jQuery(oCanvas).attr("aria-hidden", "true");
		jQuery.sap.byId(this.getId() + "-focusDummyTPEnd").attr("tabindex", "0").focusin(oRef.focusFirst);
		jQuery.sap.byId(this.getId() + "-focusDummyTPStart").attr("tabindex", "0").focusin(jQuery.proxy(this.focusPaneEnd, this));
...
sap.ui.ux3.Shell.prototype.setShowTools = function(bShowTools) {
	...
	if($SyncRefs.length){
		var oCanvas = jQuery.sap.domById(this.getId() + "-canvas");
		if(this._bRtl){
			$SyncRefs.css("right", (jQuery(document).width() - oCanvas.offsetLeft - oCanvas.offsetWidth) + "px");
		}else{
			$SyncRefs.css("left", oCanvas.offsetLeft + "px");
		}
	}
	return this;
};
...
sap.ui.ux3.Shell.prototype._adaptContentHeight = function(sNotificationBarVisibleStatus, bAnimate) {
	if(!this.getDomRef()){
		return;
	}
	...
	if(this.getFullHeightContent()){
		...
		var sTop = (iTop + (this._oFacetBar.getAssociatedItems().length > 0 ? this._iFacetBarHeight : 0)) + "px";
