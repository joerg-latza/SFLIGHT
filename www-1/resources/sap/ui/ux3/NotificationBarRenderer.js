...
(function() {
	...
	var fnRenderHoverItem = function(oRm, oControl) {
		...
		if (sStatus === "Min") {
			oRm.addStyle("top", "-"
					+ sap.ui.ux3.NotificationBar.HOVER_ITEM_HEIGHT + "px");
			oRm.addStyle("display", "block");
		} else {
			oRm.addStyle("display", "none");
		}
	...
	var fnWriteItemsMaximized = function(oRm, oControl) {
		...
		if (oMessageNotifier && oMessageNotifier.hasItems()) {
			...
			var oMNView = fnRenderNotifierMaximized(oMessageNotifier);
		...
		if (aNotifiers.length > 0) {
			for ( var i = 0; i < aNotifiers.length; i++) {
				if (aNotifiers[i].hasItems()) {
					...
					var oView = fnRenderNotifierMaximized(aNotifiers[i]);
	...
	var fnRenderNotifier = function(oRm, oNotifier, bMessageNotifier) {
		...
		oRm.writeAttribute("aria-describedby", oNotifier.getId()
				+ '-description>');
		...
		oRm.write('<div id="' + oNotifier.getId() + '-description"');
		oRm.addStyle("display", "none");
		oRm.writeStyles();
		oRm.write(">");
		...
		if (iCount > 0) {
			...
			oRm.write('<div id="' + oNotifier.getId()
					+ '-counter" role="tooltip"');
	...
	var fnRenderNotifierMaximized = function(oNotifier) {
	...
	var fnWriteNotifierIcon = function(oRm, sUri, bMessageNotifier) {
		...
		if (sUri == null || sUri == "") {
			if (bMessageNotifier) {
				iconUrl = jQuery.sap.getModulePath("sap.ui.ux3", '/')
						+ "themes/"
	 ...
	 */
	var fnRenderMessageNotifierMessageArea = function(oRm, oMessageNotifier,
			oNotiBar) {
