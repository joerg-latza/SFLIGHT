...
sap.ui.ux3.Feeder.prototype.getMultilineText = function(oInput){
	...
	// loop at children to find text elements
	var aChildren = oInput.get(0).childNodes,
		sText = "";
	...
	for ( var i = 0; i < aChildren.length; i++) {
		...
		case "#text":
			if (oChild.innerText) { //IE
				sText = sText + oChild.innerText;
			}else if (oChild.nodeValue){//Firefox, Safari
				sText = sText + oChild.nodeValue;
			}
			break;
		...
		case "P":
			if (oChild.innerText) { //IE
				sText = sText + oChild.innerText + "\n";
			}else if (oChild.textContent){
				sText = sText + oChild.textContent;
			}else{// just empty linebreak
				sText = sText + "\n";
			}
			break;
		...
		case "#comment": //Firefox - if copied and pasted from MS-Word -> ignore
			break;

		case "PRE": // copied from MS Word - add linebreak
			if (oChild.innerText) { //IE
				sText = sText + oChild.innerText + "\n";
			}else if (oChild.textContent){
				sText = sText + oChild.textContent + "\n";
		...
		default:
			// other tags (e.g. by copy&paste of HTML text into feeder) -> just check if text is there and take it
			if (oChild.innerText) { //IE
				sText = sText + oChild.innerText + "\n";
			}else if (oChild.textContent){
				sText = sText + oChild.textContent;
