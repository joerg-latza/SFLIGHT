 ...
 */
var fnFixTooLargeContent = function(oThis){
	var $this = oThis.$();
	var iHeight = $this.height();
	// subtract the gap between top and the ToolPopup
	var iTop = $this.offset().top;
	iHeight -= iTop;
	
	var iWinHeight = jQuery(window).height();
	
	if (iHeight > iWinHeight){
		var iBottomGap = parseInt($this.css("padding-bottom"));
		iBottomGap += parseInt($this.css("border-bottom-width")) + 75;
		
		iWinHeight -= iTop + iBottomGap;
		
		var $content = jQuery.sap.byId(oThis.getId() + "-content");
		$content.css("max-height", iWinHeight + "px");
...
sap.ui.ux3.ToolPopup.prototype.open = function(my, at) {
	...
	if (!this._bPositionSet) {
		...
		if (oOpener){
			...
			this.setPosition(my, at, oOpener, sOffset, "none");
	...
	this.oPopup.open(400, my, at, oOpener, sOffset, "", true);
...
var fnGetArrowDirection = function(oThis, sMy, sAt) {
	...
	if (!sMy && oThis.oPopup){
		sMy = oThis.oPopup._oPosition.my;
	}
	
	if (!sAt && oThis.oPopup){
		sAt = oThis.oPopup._oPosition.at;
	}
